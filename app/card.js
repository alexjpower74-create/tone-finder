  ...
  return (flags ?? [])
    .map(
      (f) => `<div class="ares-flag" role="note" data-testid="ares-flag">
        ...
        ${isSample(f) ? '<p class="test-only" data-testid="test-only">Test sample from the demo data — not a guide quote.</p>' : ''}
        ...
        <blockquote class="quote quote-small${isSample(f) ? ' quote-sample' : ''}" data-testid="flag-quote">
          <p class="quote-text">${esc(f.quote)}</p>${
            f.page
              ? `<p class="quote-meta">${isSample(f) ? '<span class="small">flagged on the quote from</span> ' : ''}${pagePill(f.page)}</p>`
              : ''
          }
        </blockquote>
      </div>`,
