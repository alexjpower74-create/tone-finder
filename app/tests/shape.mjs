...
class Checker {
  ...
  strings(obj, k, path) {
    if (this.key(obj, k, 'array', path)) obj[k].forEach((s, i) => this.is(s, 'string', `${path}.${k}[${i}]`));
  }
  pages(obj, k, path) {
  ...
  quote(q, path, within = null) {
    ...
    if (this.key(q, 'page', 'integer', path)) {
      ...
      if (within && (q.page < within.start || q.page > within.end)) this.fail(`${path}.page: ${q.page} outside pp. ${within.start}–${within.end}`);
    }
  }
  ...
  cab(obj, path, within) {
    ...
    if (this.key(c, 'notes', 'array', `${path}.cab`)) c.notes.forEach((n, i) => this.quote(n, `${path}.cab.notes[${i}]`, within));
  }
  facets(obj, path) {
    if (!this.key(obj, 'facets', 'object', path)) return;
    if (this.key(obj.facets, 'master_volume', 'string', `${path}.facets`)) this.oneOf(obj.facets.master_volume, MV, `${path}.facets.master_volume`);
    if (this.key(obj.facets, 'power_tubes', 'array', `${path}.facets`)) obj.facets.power_tubes.forEach((t, i) => this.oneOf(t, TUBES, `${path}.facets.power_tubes[${i}]`));
  }
}
...
export function checkModelsList(r) {
  ...
  if (c.key(r, 'models', 'array', p)) {
    ...
    r.models.forEach((s, i) => summaryInto(c, s, `${p}.models[${i}]`));
  }
...
function modelInto(c, m, p) {
  ...
  if (c.key(m, 'tips', 'array', p)) m.tips.forEach((t, i) => { c.quote(t, `${p}.tips[${i}]`, pages); c.saidBy(t, `${p}.tips[${i}]`); });
  c.cab(m, p, pages);
  ...
  if (c.key(m, 'notes', 'array', p)) {
    ...
    m.notes.forEach((n, i) => { c.quote(n, `${p}.notes[${i}]`, pages); c.saidBy(n, `${p}.notes[${i}]`); });
  }
}
...
export function checkAnswer(a) {
  ...
  if (c.key(a, 'general_knowledge', 'array', p)) {
    a.general_knowledge.forEach((g, i) => {
      if (c.key(g, 'text', 'string', `${p}.general_knowledge[${i}]`) && g.text.length > 200) c.fail(`${p}.general_knowledge[${i}].text: over 200 characters`);
      if (c.key(g, 'label', 'string', `${p}.general_knowledge[${i}]`) && g.label !== GK_LABEL) c.fail(`${p}.general_knowledge[${i}].label: must be "${GK_LABEL}"`);
    });
  }
  ...
  if (c.key(a, 'suggestions', 'array', p)) {
    ...
    a.suggestions.forEach((s, i) => {
      ...
      if (c.key(s, 'knobs', 'array', sp)) {
        ...
        if (JSON.stringify(order) !== JSON.stringify(KNOBS)) c.fail(`${sp}.knobs: order ${JSON.stringify(order)}, want exactly ${JSON.stringify(KNOBS)}`);
        s.knobs.forEach((k, j) => {
          const kp = `${sp}.knobs[${j}]`;
  ...
  if (c.key(a, 'ares', 'object', p)) {
    ...
    if (c.key(ar, 'sources', 'array', `${p}.ares`)) {
      ...
      ar.sources.forEach((s, i) => {
        if (c.key(s, 'url', 'string', `${p}.ares.sources[${i}]`) && !s.url.startsWith('https://')) c.fail(`${p}.ares.sources[${i}].url: not https`);
