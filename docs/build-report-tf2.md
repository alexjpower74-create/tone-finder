...
**Final. Stopped for good.** Merged main `24a54f4` plus the lead's doc commit (fast-forward to `69b3078`), then
rebuilt the mock from the final engine: `083d0d2` (only `app/mock/answers.json` changed).
`app/mock/models.json` is identical to `data/models.json`; `app/mock/answers.json` is generated by `core/answer.js`.
To rebuild after any data change: `node app/tools/build-mock.mjs` (defaults to `data/models.json`; checks the local
guide's sha first).

**Suite at `083d0d2`**, on the same tree as the green run at `69b3078` plus the rebuilt fixtures. It ran from my own
clean worktree on `TF_APP_PORT=8311` (8301 holds the lead's demo), chromium + webkit at 390 and 1280: **183 passed,
117 skipped** by design. The gate required a "passed" line and no "failed" line or startup error. No test depended on
the old one-card Van Halen answer, so no test changed. Not QA numbers; I didn't run `rig qa`.

**Final shots, `ask-results-390` and `-1280`:** "2 starting points for “Van Halen brown sound”".
- **Brit Brown** ("Fractal Audio custom model (no real amp) · pp. 60–61"): seven dashed guess dials, a Presence nudge
  with its p. 60 quote, and p. 61 cab notes.
- **USA Clean** ("Based on MESA/Boogie Mark IV · pp. 262–265"): why p. 265 "…replicate Van Halen’s Brown Sound or John
  Petrucci’s saturated high gain tones.", seven guesses, speaker and stock cabs p. 262, and cab notes pp. 264–265.

At 1280 the two cards fill both columns. Every quote is a teal-edged blockquote with a page pill.

## Round 4

Commits: merge of main (fast-forward to `e6a53b1`) · round 4 code and fixtures `5296419` · shots + report (this
commit).

**Suite at `5296419`, from my own clean worktree, chromium + webkit at 390 and 1280: 183 passed, 117 skipped**
(the skips are by design: run-once data and server specs, the phone spec on desktop, and the live spec). These are
not QA numbers; I didn't run `rig qa`.

**Two things went wrong on the way, both mine:**
- **I committed before any test ran.** `5296419` was committed by a script whose gate only looked for a
  "N failed" line. That run never started, because port 8301 was held by the lead's `npm run demo`
  (`scripts/demo.mjs` in the main checkout, pid 3992648), so there was no "failed" line and the gate passed. The
  four negative controls in that script were void for the same reason. I didn't touch the demo. I reran everything
  on port **8311** (`TF_APP_PORT=8311`; unassigned, not a QA port) with a gate that requires a "passed" line and no
  startup error. The numbers above and the controls below come from that rerun, on the same commit with no changes.
- **Port 8311 is outside the ports table.** The app's port is 8301, but it was taken by the lead's demo, and 8311 is
  nobody's.

| Item | What changed | Status |
|---|---|---|
| 1 | `git merge main`. `build-mock.mjs` now imports `checkQuote` (core/verify), `spansBoxBreak` with `sectionTitles(data)` and `cleanCut` (core/text, core/models), `pagesSha256` (core/guide) and `loadPages` (core/tests/guide-node). My copy `app/tools/guide-text.mjs` is deleted. The builder refuses to run if the local guide's sha ≠ `models.json`'s, and lists every stored or shown quote that doesn't verify, spans a box break, isn't a clean cut, or ends with an attribution. `quotes.spec.mjs` keeps its checks and runs them with core's functions: the §4.2 table, the offsets on all 301 pages, and every fixture quote (verified, no box break, no attribution tail, clean cut). It adds one check: with section titles, the guide has more box breaks than without, so the header and page-number breaks are live. | DONE |
| 2 | Canned answers come from `core/answer.js` (AI off): Van Halen brown sound, clean worship pad with sparkle, Robben Ford, djent, JTM 45 (guide knobs + a Bass-down nudge), Plexi crunch ("PLEXI 100W" shows "Guide section: Plexi models (Marshall Plexi)"), and AC30 chime. Hand-made on top of engine output: (a) Master of Puppets AI on, via `guideAnswer` with `extraTerms ['metallica', 'mark iic+']`, an `ai_checked` USA IIC+ pick built by `buildSuggestion` with the engine's own p. 270 "Metallica’s IIC+" citation, general knowledge, and drops `Brown Sound Deluxe` / `USA IIC+, p. 12`; AI off is the engine's no-support answer. (b) The planted SAMPLE Ares flag on the AC30 card. The no-support template is the engine's answer for "banjo through a toaster". Tests updated to the real answers: Robben Ford's first card is ODS-100; the guide-dial check uses Brit JM45; the header test uses Brit Brown, Brit JM45 and Plexi crunch. My earlier "AC30 chime" note is moot: the engine picks unit "Class-A 30W" and its own why quote. | DONE |
| 3 | The planted flag is `where: "why"` with `page` = the AC30 card's first why quote's page (107). Shape rule: `page` must be an integer for why / settings / direction and null for ai. In the UI the SAMPLE flag carries "Test sample from the demo data — not a guide quote.", a dashed amber edge, and "flagged on the quote from p. 107". | DONE |
| 4 | Every query in `answers.json` now has both `on` and `off` Answers, with no nulls. `api.mock.js` falls back to the other variant when one is missing or null, and with AI off never shows `ai.used` or general knowledge. `shape.spec` reports a null variant as a failure instead of skipping it. | DONE |
| 5 | Suite green (above), shots retaken and looked at (below), report updated. | DONE |

### Negative controls, round 4 (on 8311; each made red once, restored with `git checkout`, rerun green)

| Break | Test | Red with | Restored |
|---|---|---|---|
| AC30 why flag `page` set to null | every fixture answer has the §6 Answer shape | `ares_flags[0].page: must be the flagged quote's page for where "why"` | green |
| djent `off` variant set to null | same | `"djent" (off): missing Answer (null)` | green |
| test-sample label text removed from `card.js` | Ares: planted flag callout… (chromium-390) | `Expected "Test sample from the demo data — not a guide quote." Received "Mentioned in this suggestion."` | green |
| the glued 1959SLP tip planted in `models.json` (kept from round 3) | every quote in app/mock/models.json verifies, spans no box break… (now core's `spansBoxBreak`) | `$.models[6].tips[4] p. 28: Or just crank everything, like Eddie Van Halen “My settings…` | green |

### What the round 4 shots showed

- **`ask-results-390` and `-1280`:** the engine gives Van Halen brown sound one card, Brit Brown. Its why quotes
  are the synopsis "Custom amp models by Fractal Audio, recreating EVH’s “Brown Sound”" (p. 60) and "The Brit Brown
  and FAS Brown amp models are Fractal Audio’s take on the pure, raw guitar tones on early Van Halen albums." (p. 60).
  The card also has seven dashed guess dials, "Presence nudged up" with its p. 60 quote, and two p. 61 cab notes;
  every quote is a teal-edged blockquote with single marks. At 1280 the second column is empty because there is one
  card.
- **`model-detail-390`:** unchanged from round 3, which is expected since 1959SLP's data is the same. Byte-identical file.
