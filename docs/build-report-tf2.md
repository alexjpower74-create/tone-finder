...
**Round 4 done. Stopped.** `app/mock/models.json` is identical to `data/models.json` (merged main `e6a53b1`), and
`app/mock/answers.json` is generated by `core/answer.js`. To rebuild after any data change:
`node app/tools/build-mock.mjs` (defaults to `data/models.json`; checks the local guide's sha first).
